...
// 2. Fahrzeuge des Users laden (FÜR DAS DROPDOWN-MENÜ)
try {
    $stmtCars = $conn->prepare("SELECT id, brand, model, licenseplate FROM garage WHERE user_id = :uid ORDER BY brand, model");
    $stmtCars->execute(['uid' => $user_id]);
...
// Bestimme die Einheit (Month/Year/Day)
...
try {
    ...
    $results['kpis']['carModel'] = $vehicle['brand'] . ' ' . $vehicle['model'] . ' (' . $vehicle['licenseplate'] . ')';    
    
    // A. Kosten-KPIs (Jährlich und Monatlich)
    $total_costs_sql = "
        SELECT 
    ...
    // B. Verbrauchs-KPIs (Basisdaten für Durchschnitt)
    $consumption_sql = "
    ...
    $results = [ 
        'kpis' => [
            'carModel' => $vehicle['brand'] . ' ' . $vehicle['model'] . ' (' . $vehicle['licenseplate'] . ')',
            'averageConsumption' => 0, 'fuelCostsPerKm' => 0, 'mileage' => 0, 'range' => 0,
            'monthlyTotalCosts' => 0, 'annualTotalCosts' => 0
        ], 
